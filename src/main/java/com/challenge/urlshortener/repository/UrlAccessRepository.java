...
public interface UrlAccessRepository
    extends JpaRepository<UrlAccessEntity, Long> {

  @Query(
      value = "select * from urls_accesses u where u.url = ?1",
      nativeQuery = true)
  List<UrlAccessEntity> findByUrl(String url);

  @Query(
      value =
          "SELECT * FROM urls_accesses WHERE url_id = :urlId AND "
              + "access_date = :accessDate LIMIT 1",
      nativeQuery = true)
  Optional<UrlAccessEntity> findByUrlAndAccessDate(
      @Param("urlId") Long urlId, @Param("accessDate") LocalDate accessDate);
}
